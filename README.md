...
<p>
<a href="https://arxiv.org/abs/2507.13346">
<img src="https://img.shields.io/badge/arXiv-2507.13346-b31b1b" alt="arXiv">
</a>
<a href="https://silent-chen.github.io/AutoPartGen/">
<img src="https://img.shields.io/badge/Project_Page-green" alt="Project Page">
</a>
<a href="https://huggingface.co/spaces/facebook/autopartgen">
<img src="https://img.shields.io/badge/Hugging%20Face-Demo-blue" alt="Hugging Face Demo">
</a>
</p>
...
When using pip PyTorch wheels, the CUDA runtime headers and libraries may live
under `site-packages/nvidia/` instead of `/usr/local/cuda`. If `diso` cannot
find `cuda_runtime.h`, `thrust/device_ptr.h`, or `libcudart`, install the
matching `nvidia-cuda-*-cu12` packages and expose their `include/` and `lib/`
directories through `CPATH`, `LIBRARY_PATH`, and `LD_LIBRARY_PATH` before
running `pip install diso --no-build-isolation`.
...
```bash
python inference.py \
  --image examples/image/camera/image.png \
  --output_path outputs/camera
```
...
`mesh_001.glb`, ...) plus the combined part mesh as `mesh_combined.glb`.
Accepted parts are written as soon as they are decoded, then overwritten with
the final colored versions when generation finishes. For indexed-mask inputs,
...
image_parts = generate_from_image(
    pipeline,
    "examples/image/camera/image.png",
...
| --- | ---: | ---: |
| whole image-to-mesh stage | `whole_cfg_scale=5.0` | n/a |
| `image` | `0.0` | `5.0` |
| `mesh` | `0.0` | `3.0` |
| `image_mesh` | `0.0` | `5.0` |
| `image_mask` | `7.0` | `4.0` |
| `image_mesh_mask` | `7.0` | `4.0` |

`--image_cfg_scale`, `--geometry_cfg_scale`, `--mask_image_cfg_scale`,
`--mask_geometry_cfg_scale`, and `--whole_cfg_scale` override the config for a
single run. Treat `autopartgen/configs/default.yaml` as the source of truth if
these values change.

`--grid_size` sets the final iso-surface resolution. It must be a power of two;
use `256` for quick lower-resolution runs and `512` for release-quality output.

By default, post-processing is enabled but Taubin smoothing is not. The default
release path:

- removes floaters from each generated part;
- simplifies each part to `--simplify_faces 50000`;
- removes duplicated or empty part candidates before they are added to history
  or saved;
- runs final scene-level part cleanup;
- skips Taubin smoothing because `--smooth_iters` defaults to `0`.
...
`--duplicate_retries N` resamples a part with deterministic retry seeds when the
raw decoded geometry is empty or overlaps a previous part above
`--iou_threshold`. Floater removal and simplification run only after a candidate
passes this check.
...
`--use_coarse_bbox` enables the coarse ROI bbox crop during mesh extraction. It
is disabled by default for the release path (which keeps the global decode
bounds); pass `--use_coarse_bbox` to enable it.
